...
#else
#define Z_WILDCARD_SEARCH_QUERY_OBJ(zo) ((pcbc_wildcard_search_query_t *)zo)
#define Z_WILDCARD_SEARCH_QUERY_OBJ_P(zv) ((pcbc_wildcard_search_query_t *)zend_object_store_get_object(zv TSRMLS_CC))
#endif
...
zend_function_entry wildcard_search_query_methods[] = {
    ...
    PHP_ME(WildcardSearchQuery, jsonSerialize, ai_WildcardSearchQuery_none, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
    PHP_ME(WildcardSearchQuery, boost, ai_WildcardSearchQuery_boost, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
    PHP_ME(WildcardSearchQuery, field, ai_WildcardSearchQuery_field, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
    PHP_FE_END
};
// clang-format on
...
static void wildcard_search_query_free_object(pcbc_free_object_arg *object TSRMLS_DC) /* {{{ */
{
...
static pcbc_create_object_retval wildcard_search_query_create_object(zend_class_entry *class_type TSRMLS_DC)
{
...
#else
    {
        ...
        ret.handle = zend_objects_store_put(obj, (zend_objects_store_dtor_t)zend_objects_destroy_object,
                                            wildcard_search_query_free_object, NULL TSRMLS_CC);
        ret.handlers = &wildcard_search_query_handlers;
        return ret;
    }
#endif
}
...
PHP_MINIT_FUNCTION(WildcardSearchQuery)
{
    ...
    PCBC_CE_FLAGS_FINAL(pcbc_wildcard_search_query_ce);
    PCBC_CE_DISABLE_SERIALIZATION(pcbc_wildcard_search_query_ce);

    zend_class_implements(pcbc_wildcard_search_query_ce TSRMLS_CC, 1, php_json_serializable_ce);
