...
#else
#define Z_MATCH_ALL_SEARCH_QUERY_OBJ(zo) ((pcbc_match_all_search_query_t *)zo)
#define Z_MATCH_ALL_SEARCH_QUERY_OBJ_P(zv) ((pcbc_match_all_search_query_t *)zend_object_store_get_object(zv TSRMLS_CC))
#endif
...
PHP_METHOD(MatchAllSearchQuery, __construct) { throw_pcbc_exception("Accessing private constructor.", LCB_EINVAL); }
/* }}} */
...
zend_function_entry match_all_search_query_methods[] = {
    ...
    PHP_ME(MatchAllSearchQuery, jsonSerialize, ai_MatchAllSearchQuery_none, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
    PHP_ME(MatchAllSearchQuery, boost, ai_MatchAllSearchQuery_boost, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
    PHP_FE_END
};
// clang-format on
...
static void match_all_search_query_free_object(pcbc_free_object_arg *object TSRMLS_DC) /* {{{ */
{
...
static pcbc_create_object_retval match_all_search_query_create_object(zend_class_entry *class_type TSRMLS_DC)
{
...
#else
    {
        ...
        ret.handle = zend_objects_store_put(obj, (zend_objects_store_dtor_t)zend_objects_destroy_object,
                                            match_all_search_query_free_object, NULL TSRMLS_CC);
        ret.handlers = &match_all_search_query_handlers;
        return ret;
    }
#endif
}
...
PHP_MINIT_FUNCTION(MatchAllSearchQuery)
{
    ...
    PCBC_CE_FLAGS_FINAL(pcbc_match_all_search_query_ce);
    PCBC_CE_DISABLE_SERIALIZATION(pcbc_match_all_search_query_ce);

    zend_class_implements(pcbc_match_all_search_query_ce TSRMLS_CC, 1, php_json_serializable_ce);
