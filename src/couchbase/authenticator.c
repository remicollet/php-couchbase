...
/* {{{ proto void Authenticator::__construct() */
PHP_METHOD(Authenticator, __construct)
{
    pcbc_authenticator_t *obj;

    obj = Z_AUTHENTICATOR_OBJ_P(getThis());
...
/* {{{ proto \Couchbase\Authenticator Authenticator::cluster(string $username, string $password)
 */
PHP_METHOD(Authenticator, cluster)
{
    pcbc_authenticator_t *obj;
...
/* {{{ proto \Couchbase\Authenticator Authenticator::bucket(string $name, string $password)
 */
PHP_METHOD(Authenticator, bucket)
{
    pcbc_authenticator_t *obj;
...
void pcbc_generate_lcb_auth(pcbc_authenticator_t *auth, lcb_AUTHENTICATOR **result, lcb_type_t type, const char *name,
                            const char *password, char **hash TSRMLS_DC)
{
    PHP_MD5_CTX md5;
...
ZEND_BEGIN_ARG_INFO_EX(ai_Authenticator_none, 0, 0, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(ai_Authenticator_cluster, 0, 0, 2)
ZEND_ARG_INFO(0, username)
ZEND_ARG_INFO(0, password)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(ai_Authenticator_bucket, 0, 0, 2)
...
zend_function_entry authenticator_methods[] = {
    PHP_ME(Authenticator, __construct, ai_Authenticator_none, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL | ZEND_ACC_CTOR)
    PHP_ME(Authenticator, cluster, ai_Authenticator_cluster, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
    PHP_ME(Authenticator, bucket, ai_Authenticator_bucket, ZEND_ACC_PUBLIC | ZEND_ACC_FINAL)
    PHP_FE_END
};
// clang-format on

zend_object_handlers authenticator_handlers;

static void authenticator_free_object(pcbc_free_object_arg *object TSRMLS_DC) /* {{{ */
{
    pcbc_authenticator_t *obj = Z_AUTHENTICATOR_OBJ(object);
    pcbc_credential_t *ptr;
...
{
    pcbc_authenticator_t *obj = NULL;

    obj = PCBC_ALLOC_OBJECT_T(pcbc_authenticator_t, class_type);

    zend_object_std_init(&obj->std, class_type TSRMLS_CC);
    object_properties_init(&obj->std, class_type);

#if PHP_VERSION_ID >= 70000
    obj->std.handlers = &authenticator_handlers;
    return &obj->std;
#else
    {
        zend_object_value ret;
        ret.handle = zend_objects_store_put(obj, (zend_objects_store_dtor_t)zend_objects_destroy_object,
                                            authenticator_free_object, NULL TSRMLS_CC);
        ret.handlers = &authenticator_handlers;
        return ret;
    }
#endif
}

static HashTable *pcbc_authenticator_get_debug_info(zval *object, int *is_temp TSRMLS_DC) /* {{{ */
{
    pcbc_authenticator_t *obj = NULL;
#if PHP_VERSION_ID >= 70000
    zval retval;
    ...
    *is_temp = 1;
    obj = Z_AUTHENTICATOR_OBJ_P(object);
...
PHP_MINIT_FUNCTION(Authenticator)
{
    ...
    INIT_NS_CLASS_ENTRY(ce, "Couchbase", "Authenticator", authenticator_methods);
    pcbc_authenticator_ce = zend_register_internal_class(&ce TSRMLS_CC);
    pcbc_authenticator_ce->create_object = authenticator_create_object;
    PCBC_CE_FLAGS_FINAL(pcbc_authenticator_ce);
    PCBC_CE_DISABLE_SERIALIZATION(pcbc_authenticator_ce);

    memcpy(&authenticator_handlers, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
    authenticator_handlers.get_debug_info = pcbc_authenticator_get_debug_info;
#if PHP_VERSION_ID >= 70000
    authenticator_handlers.free_obj = authenticator_free_object;
    authenticator_handlers.offset = XtOffsetOf(pcbc_authenticator_t, std);
#endif

    zend_register_class_alias("\\CouchbaseAuthenticator", pcbc_authenticator_ce);
    return SUCCESS;
}
